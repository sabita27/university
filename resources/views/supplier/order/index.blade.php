                                            ...
                                            <div class="table-responsive">
                                                <table class="table" style="margin: 0; font-size: 13px;">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td style="padding: 14px 20px; vertical-align: middle;">{{ $key + 1 }}</td>
                                                            <td style="padding: 14px 20px; vertical-align: middle; font-weight: 600; color: #0d6efd;">{{ $order->procurement_number ?? '-' }}</td>
                                                            <td style="padding: 14px 20px; vertical-align: middle;">
                                                                @if($order->assets && count($order->assets) > 0)
                                                                    @foreach($order->assets as $asset)
                                                                        <span style="display:inline-block; background:#e8f0fe; color:#0d6efd; border-radius:4px; padding:2px 8px; font-size:11px; font-weight:600; margin:1px;">
                                                                            {{ $asset['type_title'] ?? $asset['brand_title'] ?? 'Item' }} (Qty: {{ $asset['quantity'] ?? '-' }})
                                                                        </span>
                                                                    @endforeach
                                                                @else
                                                            ...
                                                            <td style="padding: 14px 20px; vertical-align: middle; color: #555;">{{ $order->name ?? '-' }}</td>
                                                            <td style="padding: 14px 20px; vertical-align: middle; color: #555;">
                                                                {{ $order->request_date ? $order->request_date->format('Y-m-d') : '-' }}
                                                            </td>
                                                            <td style="padding: 14px 20px; vertical-align: middle;">
                                                                @php $urgency = $order->urgency ?? 'Normal'; @endphp
                                                                @if(str_contains(strtolower($urgency), 'urgent') || str_contains(strtolower($urgency), '24') || str_contains(strtolower($urgency), '48'))
                                                                    <span style="background:#dc3545; color:#fff; padding:3px 10px; border-radius:4px; font-size:11px; font-weight:700;">{{ $urgency }}</span>
                                                                @elseif(str_contains(strtolower($urgency), '7'))
                                                                    <span style="background:#fd7e14; color:#fff; padding:3px 10px; border-radius:4px; font-size:11px; font-weight:700;">{{ $urgency }}</span>
                                                                @else
                                                                    <span style="background:#6c757d; color:#fff; padding:3px 10px; border-radius:4px; font-size:11px; font-weight:700;">{{ $urgency }}</span>
                                                                @endif
                                                            </td>
                                                            <td style="padding: 14px 20px; vertical-align: middle;">
                                                            ...
                                                            <td style="padding: 14px 20px; vertical-align: middle;">
                                                                <button type="button"
                                                                    ...
                                                                    data-order-number="{{ $order->procurement_number }}"
                                                                    data-name="{{ $order->name }}"
                                                                    data-purpose="{{ $order->purpose }}"
                                                                    data-date="{{ $order->request_date ? $order->request_date->format('Y-m-d') : '-' }}"
                                                                    data-urgency="{{ $order->urgency ?? 'Normal' }}"
                                                                    data-status="{{ $order->status }}"
                                                                    data-assets="{{ json_encode($order->assets ?? []) }}"
                                ...
                                <div class="col-sm-12 mb-4">
                                    <div class="card" style="border-radius: 10px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); border: none;">
                                        ...
                                        <div class="card-body" style="padding: 0;">
                                            <div class="table-responsive">
                                                <table class="table" style="margin: 0; font-size: 13px;">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td style="padding: 14px 20px; vertical-align: middle;">{{ $orders->firstItem() + $key }}</td>
                                                            <td style="padding: 14px 20px; vertical-align: middle; color: #0d6efd; font-weight: 600;">{{ $order->procurement_number ?? '-' }}</td>
                                                            <td style="padding: 14px 20px; vertical-align: middle;">
                                                                @if($order->assets && count($order->assets) > 0)
                                                                    @foreach($order->assets as $asset)
                                            ...
                                            @if($orders->hasPages())
                                            <div style="padding: 15px 20px;">{{ $orders->links() }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
