...
<div class="main-body">
    <div class="page-wrapper">
        ...
        <div class="procurement-card">
            <form method="POST" action="{{ route('admin.procurement.store') }}" class="needs-validation" novalidate>
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control readonly-bg" name="procurement_number" id="procurement_number" value="{{ old('procurement_number', $generated_procurement_number) }}" readonly required>
                        ...
                        <select class="form-control select2" name="staff_id" id="supplier_select">
                            ...
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" data-department-id="{{ $supplier->department_id }}"
                                    @if(old('staff_id') == $supplier->id) selected @endif>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-4 mb-3">
                        ...
                        <input type="date" class="form-control" name="request_date" id="request_date" value="{{ old('request_date', date('Y-m-d')) }}" required>
                    ...
                    <div class="form-group col-md-4 mb-3">
                        ...
                        <select class="form-control" name="urgency" id="urgency" required>
                            ...
                            <option value="Immediate" @if(old('urgency') == 'Immediate') selected @endif>Immediate</option>
                            <option value="Within 7 days" @if(old('urgency') == 'Within 7 days') selected @endif>Within 7 days</option>
                            <option value="Within 15 days" @if(old('urgency') == 'Within 15 days') selected @endif>Within 15 days</option>
                            <option value="Within 30 days" @if(old('urgency') == 'Within 30 days') selected @endif>Within 30 days</option>
                            <option value="Other" @if(old('urgency') == 'Other') selected @endif>Other</option>
                        </select>
                        <div class="form-group mt-2" id="urgency_reason_div" style="display: {{ old('urgency') == 'Other' ? 'block' : 'none' }};">
                            <label for="urgency_reason" class="form-label">Urgency Reason <span>*</span></label>
                            <input type="text" class="form-control" name="urgency_reason" id="urgency_reason" value="{{ old('urgency_reason') }}" {{ old('urgency') == 'Other' ? 'required' : '' }}>
                        </div>
                    ...
                    <div class="form-group col-md-4 mb-3">
                        ...
                        <textarea class="form-control" name="purpose" id="purpose" rows="2" placeholder="Enter Purpose">{{ old('purpose') }}</textarea>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <button type="submit" class="btn btn-add-request"><i class="fas fa-check"></i> {{ __('btn_save') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if ($departmentSelect.length && $staffSelect.length) {
            function filterStaff() {
                ...
                const currentStaffId = $staffSelect.val() || "{{ old('staff_id') }}";
        ...
        @if(old('assets'))
            try {
                assets = JSON.parse({!! json_encode(old('assets')) !!});
