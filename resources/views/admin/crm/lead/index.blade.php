...
<div class="kb-header-top">
    ...
    <div class="kb-top-avatars">
        @php $colors = ['#8b5cf6','#f59e0b','#14b8a6','#ec4899','#3b82f6']; @endphp
        @foreach($users->take(4) as $idx => $u)
            <div class="kb-av kb-top-user-av" 
                 style="background: {{ $colors[$idx % count($colors)] }};" 
                 title="{{ $u->first_name }} {{ $u->last_name }}"
                 onclick="quickFilterMember('{{ $u->id }}', '{{ $u->first_name }} {{ $u->last_name }}')">
    ...
    <div class="kb-filter-group">
        ...
        <div class="kb-filter-item dropdown">
            ...
            <ul class="dropdown-menu border-0" style="min-width: 220px; max-height: 300px; overflow-y: auto;">
                ...
                @foreach($users as $u)
                <li><a class="dropdown-item py-2 kb-member-filter" href="#" data-uid="{{ $u->id }}">{{ $u->first_name }} {{ $u->last_name }}</a></li>
                @endforeach
            </ul>
        </div>
...
{{-- GLOBAL MODALS AT ROOT LEVEL --}}
@foreach($rows as $row)
    {{-- Delete Modal --}}
    @can($access.'-delete')
    <div class="modal fade" id="deleteCardModal-{{ $row->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content border-0 shadow-lg" style="border-radius:20px;">
            <div class="modal-body text-center p-4">
                ...
                <p class="text-muted small mb-4">Permanently delete <strong>{{ $row->name }}</strong>?</p>
...
{{-- Create Status Modal (Add Column) --}}
...
<div class="modal-content" style="border-radius:18px;overflow:hidden;border:none;">
    ...
    <form action="{{ route('admin.crm.lead-status.store') }}" method="post">
    @csrf
    <div class="modal-body p-4">
        <div class="mb-3">
            <label class="form-label fw-bold">Column Name <span class="text-danger">*</span></label>
...
(function(){
    ...
    window.assignLeadStaff = function(leadId, userId, initial, color, fname, fullName) {
        ...
        fetch("{{ route('admin.crm.lead.assign-staff') }}", {
            method: 'POST',
