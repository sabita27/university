...
Route::get('form-test', 'App\Http\Controllers\Web\LeadController@index');

// Admission Inquiry Route
// Web Routes
Route::middleware(['XSS'])->namespace('Web')->group(function () {

    // Admission Inquiry Route and public form
    Route::get('public/join-us', 'LeadController@index')->name('admission.inquiry.index');
    Route::post('public/join-us', 'LeadController@store')->name('admission.request.store');
    ...
    Route::get('/supplier-register', 'SupplierRegisterController@create')->name('supplier.register')->middleware('supplier.register');
    Route::post('/supplier-register', 'SupplierRegisterController@store')->name('supplier.register.store')->middleware('supplier.register');
...
Route::middleware(['auth:web', 'XSS'])->name('admin.')->namespace('Admin')->prefix('admin')->group(function () {
    ...
    Route::prefix('crm')->name('crm.')->group(function () {
        Route::get('enquiry', 'CrmLeadController@enquiry')->name('lead.enquiry');
        Route::get('follow-up-list', 'CrmLeadController@followUpList')->name('lead.follow-up-list');
        Route::get('lead-import', 'CrmLeadController@import')->name('lead.import');
        Route::post('lead-import', 'CrmLeadController@importStore')->name('lead.import.store');
        Route::get('lead-export', 'CrmLeadController@export')->name('lead.export');
        Route::resource('lead', 'CrmLeadController');
        Route::post('lead/{id}/update-status', 'CrmLeadController@updateStatus')->name('lead.update-status');
        Route::post('lead/assign-staff', 'CrmLeadController@assignStaff')->name('lead.assign-staff');
        Route::resource('lead-source', 'CrmLeadSourceController');
        Route::resource('lead-status', 'CrmLeadStatusController');
        Route::resource('qualification', 'CrmQualificationController');
        Route::post('lead/add-follow-up', 'CrmLeadController@addFollowUp')->name('lead.add-follow-up');
        Route::put('follow-up/{id}', 'CrmLeadController@followUpUpdate')->name('lead.follow-up.update');
        Route::delete('follow-up/{id}', 'CrmLeadController@followUpDestroy')->name('lead.follow-up.destroy');
    });



    // Procurement Routes
    Route::post('procurement/{procurement}/approve', 'ProcurementController@approve')->name('procurement.approve');
    Route::resource('procurement', 'ProcurementController');



    // Asset Management Routes
    Route::resource('asset/brand', 'AssetBrandController')->names([
        'index'   => 'asset-brand.index',
        'store'   => 'asset-brand.store',
        'update'  => 'asset-brand.update',
        'destroy' => 'asset-brand.destroy',
    ]);
    Route::get('asset/brand-status/{id}', 'AssetBrandController@status')->name('asset-brand.status');
    Route::resource('asset/type', 'AssetTypeController')->names([
        'index'   => 'asset-type.index',
        'store'   => 'asset-type.store',
        'update'  => 'asset-type.update',
        'destroy' => 'asset-type.destroy',
    ]);
    Route::get('asset/type-status/{id}', 'AssetTypeController@status')->name('asset-type.status');
    Route::get('asset/working-status/{id}', 'AssetController@workingStatus')->name('asset.working-status');
    Route::get('asset/availability-status/{id}', 'AssetController@availabilityStatus')->name('asset.availability-status');
    Route::resource('asset', 'AssetController');
    Route::resource('asset-assignment', 'AssetAssignmentController');
...
Route::middleware(['auth:supplier', 'XSS'])->prefix('supplier')->name('supplier.')->namespace('Supplier')->group(function () {
    // Dashboard Route
    Route::get('/', 'DashboardController@index')->name('dashboard.index');
    Route::get('dashboard', 'DashboardController@index')->name('dashboard.index');
